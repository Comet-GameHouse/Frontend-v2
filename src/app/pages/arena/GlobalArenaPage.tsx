...
import type { MatchSummary, RoomParticipant } from '@types'
...
function GlobalArenaPage() {
  ...
  const readyPlayers = matchSummary.participants.filter(
  ...
  return (
    ...
    >
      <section className="grid gap-6 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <button
              type="button"
              className="rounded-lg border border-slate-700 px-4 py-2 text-slate-300 transition hover:border-slate-600 hover:text-slate-100"
            >
              Watch Live
            </button>
          </div>
      ...
      <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-6 text-slate-300">
        ...
        <p className="mt-3">
          Every hour, the arena spins up a fresh rule set. This session rewards aggressive play with
          extra shard drops. Jump in solo or bring a squad to dominate the global stage.
        </p>
      </section>
    </GameLayout>
  )
