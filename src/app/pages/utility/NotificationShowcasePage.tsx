  ...
  return (
    ...
    >
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {demoNotifications.map((item) => (
          <button
            ...
            className="rounded-2xl border border-slate-800 bg-slate-900/50 px-4 py-5 text-left text-slate-200 transition hover:border-slate-700 hover:bg-slate-900"
          >
      ...
      <button
        type="button"
        onClick={clearAll}
        className="mt-6 inline-flex items-center gap-2 rounded-full border border-slate-700 px-5 py-2 text-sm font-semibold uppercase tracking-[0.3em] text-slate-300 transition hover:border-slate-500 hover:text-slate-100"
      >
        Clear All
      </button>
    </UtilityLayout>
